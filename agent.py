...
from buffer import ReplayBufferDeque, ReplayBufferManual, ReplayBufferDeque2
import random
...
class DQN_Agent():
    ...
    Args:
        ...
        file_pth (str): 
            Directory path to save models, logs, and configurations.
        model_args (dict): 
            Arguments for constructing the actor and critic networks.
        lr (float): 
            Learning rate for the q-network. Defaults to 1e-3.
        buffer_size (int): 
            Maximum size of the replay buffer. Defaults to 1,000,000.
        learning_starts (int): 
            Number of steps before training starts and transitions are sampled. Defaults to 100.
        batch_size (int): 
            Size of minibatch sampled from the replay buffer. Defaults to 256.
        tau (float): 
            Target smoothing coefficient (Polyak averaging). Defaults to 0.005.
        gamma (float): 
            Discount factor for future rewards. Defaults to 0.99.
        optimizer_type (str): 
            Optimizer to use ('Adam', 'SGD', etc.). Defaults to 'Adam'.
        epsilon (float): 
            Initial epsilon for epsilon-greedy exploration. Defaults to 1.0.
        epsilon_min (float): 
            Minimum epsilon value. Defaults to 0.01.
        epsilon_min_ep (int): 
            Episode at which epsilon reaches epsilon_min. Defaults to None.
        epsilon_decay (float): 
            Multiplicative decay factor for epsilon per episode. Defaults to 0.99.
        step_repeat (int): 
            Number of times to repeat each action. Defaults to 1.
        target_update_interval (int): 
            Steps between target network updates. Defaults to 1.
        target_update_method (str): 
            Update method ('hard' or 'soft'). Defaults to 'hard'.
        seed (int, optional): 
    ...
    def __init__(
        ...
        if self.seed is not None:
            random.seed(self.seed) # seeds python's default rng
            np.random.seed(self.seed) # seeds numpy's rng
            torch.manual_seed(self.seed) # seeds CPU and current CUDA device
            torch.cuda.manual_seed_all(self.seed) # seeds all CUDA device; keep just in case
            torch.backends.cudnn.deterministic = True # for cudnn
            torch.backends.cudnn.benchmark = False # for cudnn
            env.action_space.seed(self.seed) # seeds gymnasium env action
    ...
    def choose_action(self, state, epsilon):
        ...
        Args:
            env (gymnasium environment): 
                The environment, which provides the action space.
            state (torch.Tensor): 
                The input state, shape `(state_dim,)` or `(c, h, w)`.
            epsilon (float): 
        ...
        if np.random.random() < epsilon:
            ...
            with torch.no_grad():
                ...
                # squeeze to remove batch dimension and move to cpu for numpy scalar conversion later
                action = torch.argmax(q_values).squeeze(0).detach().cpu() # DONT NEED .detach BC ALREADY DID .no_grad(); make sure of this
            
        return action
        
    def compute_qvals(self, state, target=False):
        """
        Compute Q-values for given state(s) using either the policy network 
        or the target network.

        Args:
            state (torch.Tensor):
                The input state (or batch of states).
                Shape typically `(batch_size, state_dim)`.
            target (bool, optional (default=False)):
                If True, Q-values are computed using the target network (`self.target_nn`).
                If False, Q-values are computed using the policy (online) network 
                (`self.policy_nn`).

        Returns:
            q_values (torch.Tensor):
                The predicted Q-values for each action. Shape typically 
    ...
    def update_q(self, states, actions, next_states, rewards, terminations):
        """
        Performs an update of the Q-network.

        Args:
            states (torch.Tensor): 
                Batch of current states, shape `(batch_size, state_dim)`.
            actions (torch.Tensor): 
                Batch of actions taken, shape `(batch_size,)`.
            next_states (torch.Tensor): 
                Batch of next states, shape `(batch_size, state_dim)`.
            rewards (torch.Tensor): 
                Batch of rewards received, shape `(batch_size,)`.
            terminations (torch.Tensor): 
                Batch of done flags (1 if terminal), shape `(batch_size,)`.

        Returns:
            losses (torch.Tensor): 
                The loss, defined by `self.loss_fn`, for given batch.
        """
        # extract the qval prediction corresponding to each state, action pair; shape (batch_size,)
    ...
    def train(self, total_steps, num_envs=None): # DQNAgent's train method does not support vectorized envs but will be set as None for generability
        ...
        for step in tqdm(range(total_steps), desc="Training steps"):
            action = self.choose_action(state.to(self.device), self.epsilon) 
                
            reward = 0
            # OBSOLETE FEATURE; consider self.step_repeat as always 1
...
class TD3_Agent():
    ...
    Args:
        ...
        model_args (dict): 
            Arguments for constructing the actor and critic networks.
        lr_critic (float): 
            Learning rate for the critic network. Defaults to 1e-3.
        lr_actor (float): 
            Learning rate for the actor network. Defaults to 1e-3.
        buffer_size (int): 
            Maximum size of the replay buffer. Defaults to 1,000,000.
        learning_starts (int): 
            Number of steps before training starts and transitions are sampled. Defaults to 100.
        ...
        optimizer_type (str): 
            Optimizer to use ('Adam', 'SGD', etc.). Defaults to 'Adam'.
        train_freq (int): 
            Number of steps between gradient updates. Defaults to 1.
        gradient_steps (int): 
            Number of gradient updates per training step. Defaults to 1.
        noise_type (str): 
            Type of exploration noise to add to actions ('Gaussian' or 'Ornstein-Uhlenbeck'). Defaults to 'Gaussian'.
        noise_std (float): 
            Standard deviation of the exploration noise. Defaults to 0.1.
        policy_delay (int): 
            Frequency of delayed policy updates relative to critic updates. Defaults to 2.
        seed (int, optional): 
            Random seed for reproducibility. Defaults to None.
    ...
    def __init__(
        ...
        if isinstance(self.env, vector.VectorEnv): # check if vectorized env
            state, _ = self.env.reset()
            state = preprocess_data(state, model_args)
    ...
    def compute_action_old(self, state, rand=False):
        """
    ...
    def compute_action(self, state, rand=False):
        """
        Computes a continuous action with exploratory Gaussian noise.
    ...
    def update_policy(self, states):
        ...
        Args:
            states (torch.Tensor): 
                Batch of environment states, shape `(batch_size, state_dim)`.
        ...
        for step in tqdm(range(total_steps), desc="Training steps"):
            ...
            if np.any(done):
                ...
                if int(ep_rew_mean) > best_rew_mean:
                    best_rew_mean = int(ep_rew_mean)
                    print(f"\nBest reward so far! Completed episode {episode} with score {best_rew_mean}! "
                        f"Elapsed time: {elapsed_time} seconds. Step: {step}.")
                    self.actor.save_model(self.file_pth)
